...
/** @typedef {{id: number, order_number: string, status: string, ordered_at: string|null, expected_delivery_date: string|null, supplier: {name: string}|null}} PurchaseOrderRecord */
/**
 * Lapozott Inertia-adathalmaz.
 * @typedef {Object} PaginatedResult
 * @property {PurchaseOrderRecord[]} data Az aktuális oldal beszerzési rendelései.
 ...
 * @property {PaginatedResult} records A lapozott beszerzési rendelések.
...
const supplierId = ref(
    props.filters.supplier_id ? Number(props.filters.supplier_id) : null,
);
const perPage = ref(
...
const reload = (pageNumber = 1) =>
    router.get(route("admin.purchase-orders.index"), query(pageNumber), {
        preserveState: true,
        replace: true,
    });
...
<template>
    ...
    <AdminLayout>
        ...
        <div class="space-y-4">
            <AdminPageHeader
                title=""
                title-key="procurement.purchase_orders.title"
                subtitle-key="procurement.purchase_orders.subtitle"
                :can-create="false"
            />
            <AdminSearchBar
                v-model="search"
            ...
            >
                <Column
                    field="order_number"
                    :header="trans('fields.order')"
                    sortable
                    ><template #body="{ data }"
                        ><Link
                            :href="route('admin.purchase-orders.show', data.id)"
                            class="font-medium text-blue-700 hover:underline"
                            >{{ data.order_number }}</Link
                        ></template
                    ></Column
                >
                <Column :header="trans('fields.supplier')"
                    ><template #body="{ data }">{{
                        data.supplier?.name || "-"
                    }}</template></Column
                >
                ...
                <Column
                    ...
                    :header="trans('fields.expected')"
                    sortable
                    ><template #body="{ data }">{{
                        dateValue(data.expected_delivery_date)
                    }}</template></Column
                >
            </DataTable>
        </div>
