...
const generate = () => confirm.require({ message: 'Generate a consolidated requisition from current shortages?', header: 'Generate requisition', icon: 'pi pi-list-check', accept: () => router.post(route('admin.purchase-requisitions.generate-from-material-requirements')) });
const destroyRecord = (record) => confirm.require({ message: `Delete ${record.requisition_number}?`, header: 'Delete requisition', icon: 'pi pi-exclamation-triangle', acceptClass: 'p-button-danger', accept: () => router.delete(route('admin.purchase-requisitions.destroy', record.id)) });
...
<template>
    ...
    <AdminLayout>
        ...
        <ConfirmDialog />
        <div class="space-y-4">
            <AdminPageHeader title="Purchase Requisitions" subtitle="Consolidated purchase demand with source material requirements." create-label="Generate from shortages" @create="generate" />
            <AdminSearchBar v-model="search" v-model:per-page="perPage" @search="reload(1)" />
            <div class="rounded border border-slate-200 bg-white p-3">
                <Select v-model="status" :options="statusOptions" option-label="label" option-value="value" placeholder="Status" show-clear class="w-full sm:w-72" @change="reload(1)" />
            </div>
            <DataTable :value="records.data" lazy paginator :rows="records.per_page" :first="(records.current_page - 1) * records.per_page" :total-records="records.total" :sort-field="sortField" :sort-order="sortOrder" data-key="id" class="rounded border border-slate-200 bg-white" @page="(event) => { perPage = event.rows; reload(event.page + 1); }" @sort="(event) => { sortField = event.sortField; sortOrder = event.sortOrder; reload(1); }">
                <Column field="requisition_number" header="Requisition" sortable><template #body="{ data }"><Link :href="route('admin.purchase-requisitions.show', data.id)" class="font-medium text-blue-700 hover:underline">{{ data.requisition_number }}</Link></template></Column>
                ...
                <Column field="requested_at" header="Requested" sortable><template #body="{ data }">{{ dateValue(data.requested_at) }}</template></Column>
                <Column header="" body-style="text-align:right"><template #body="{ data }"><Button type="button" icon="pi pi-trash" text rounded severity="danger" aria-label="Delete" @click="destroyRecord(data)" /></template></Column>
            </DataTable>
        </div>
    </AdminLayout>
</template>
