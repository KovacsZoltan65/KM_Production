...
/** @typedef {{id: number, requisition_number: string, status: string, requested_at: string|null, needed_by_date: string|null}} PurchaseRequisitionRecord */
/**
 * Lapozott Inertia-adathalmaz.
 * @typedef {Object} PaginatedResult
 * @property {PurchaseRequisitionRecord[]} data Az aktuális oldal beszerzési igényei.
...
const reload = (pageNumber = 1) =>
    router.get(route("admin.purchase-requisitions.index"), query(pageNumber), {
        preserveState: true,
        replace: true,
    });
const generate = () =>
    confirm.require({
        message: trans(
            "procurement.purchase_requisitions.confirm_generate_message",
        ),
        header: trans(
            "procurement.purchase_requisitions.confirm_generate_header",
        ),
...
const destroyRecord = (record) =>
    confirm.require({
        message: trans("confirm.delete_named_message", {
            name: record.requisition_number,
        }),
        header: trans(
            "procurement.purchase_requisitions.confirm_delete_header",
        ),
        icon: "pi pi-exclamation-triangle",
        acceptClass: "p-button-danger",
        accept: () =>
            router.delete(
                route("admin.purchase-requisitions.destroy", record.id),
            ),
    });
...
<template>
    ...
    <AdminLayout>
        ...
        <div class="space-y-4">
            <AdminPageHeader
                title=""
                title-key="procurement.purchase_requisitions.title"
            ...
            >
                <Column
                    ...
                    :header="trans('fields.requisition')"
                    sortable
                    ><template #body="{ data }"
                        ><Link
                            :href="
                ...
                <Column
                    field="requested_at"
                    :header="trans('fields.requested')"
                    sortable
                    ><template #body="{ data }">{{
                        dateValue(data.requested_at)
                    }}</template></Column
                >
                <Column header="" body-style="text-align:right"
                    ><template #body="{ data }"
                        ><Button
                            ...
                            @click="destroyRecord(data)" /></template
                ></Column>
            </DataTable>
        </div>
    </AdminLayout>
