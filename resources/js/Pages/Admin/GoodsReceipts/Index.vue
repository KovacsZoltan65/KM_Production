...
const form = useForm({
    ...
    notes: "",
    items: [{ item_id: null, location_id: null, quantity: 1, notes: "" }],
});
...
const reload = (pageNumber = 1) =>
    router.get(route("admin.goods-receipts.index"), query(pageNumber), {
        preserveState: true,
        replace: true,
    });
...
<template>
    ...
    <AdminLayout>
                ...
                <Column field="receipt_number" :header="$t('fields.receipt')" sortable
                    ><template #body="{ data }"
                        ><Link
                            ...
                            >{{ data.receipt_number }}</Link
                        ></template
                    ></Column
                >
                <Column :header="$t('fields.supplier')"
                    ><template #body="{ data }">{{
                        data.purchase_order?.supplier?.name || "-"
                    }}</template></Column
                >
                <Column :header="$t('fields.purchase_order')"
                    ><template #body="{ data }">{{
        ...
        <Dialog
            ...
            modal
            :header="$t('procurement.goods_receipts.create_title')"
        ...
        >
            <form class="space-y-4" @submit.prevent="submit">
                ...
                <div
                    ...
                    :key="index"
                    class="grid gap-3 rounded border border-slate-200 p-3 md:grid-cols-[1fr_1fr_8rem]"
                >
                    <Select
