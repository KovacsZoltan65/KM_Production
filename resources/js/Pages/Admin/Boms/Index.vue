...
const reload = (pageNumber = 1) => {
    router.get(route("admin.boms.index"), query(pageNumber), {
        preserveState: true,
        replace: true,
    });
};
...
const destroyRecord = (record) => {
    confirm.require({
        ...
        accept: () =>
            router.delete(route("admin.boms.destroy", record.id), {
                preserveScroll: true,
            }),
    });
};
    ...
    <AdminLayout>
        ...
        <div class="space-y-4">
            <AdminPageHeader
                title=""
                title-key="bom.title"
                subtitle-key="bom.subtitle"
                create-label-key="bom.actions.create"
                @create="openCreate"
            />
            <AdminSearchBar
                v-model="search"
