    ...
    <form class="space-y-4" @submit.prevent="submit">
        <div class="grid gap-3 sm:grid-cols-2">
            <div class="space-y-1">
                ...
                <Select
                    ...
                    class="w-full"
                />
                <Message v-if="form.errors.document_type" severity="error" size="small">{{ form.errors.document_type }}</Message>
            </div>

            <div class="space-y-1">
                <label class="text-sm font-medium text-slate-700">Linked entity</label>
                <Select
                    ...
                    class="w-full"
                />
                <Message v-if="form.errors.documentable_type" severity="error" size="small">{{ form.errors.documentable_type }}</Message>
            </div>
        </div>

        <div class="grid gap-3 sm:grid-cols-[12rem_1fr]">
            <div class="space-y-1">
                <label class="text-sm font-medium text-slate-700">Entity ID</label>
                <InputNumber v-model="form.documentable_id" input-class="w-full" class="w-full" :min="1" />
                <Message v-if="form.errors.documentable_id" severity="error" size="small">{{ form.errors.documentable_id }}</Message>
            </div>
        ...
        <div class="space-y-1">
            ...
            <input
                ...
                @change="pickFile"
            />
            <Message v-if="form.errors.file" severity="error" size="small">{{ form.errors.file }}</Message>
        </div>

        <div class="space-y-1">
            <label class="text-sm font-medium text-slate-700">Notes</label>
            <Textarea v-model="form.notes" rows="4" class="w-full" />
            <Message v-if="form.errors.notes" severity="error" size="small">{{ form.errors.notes }}</Message>
        </div>

        <div class="flex justify-end gap-2">
            <Button type="button" label="Cancel" severity="secondary" outlined @click="$emit('cancel')" />
            <Button type="submit" label="Upload" icon="pi pi-upload" :loading="form.processing" />
        </div>
    </form>
</template>
