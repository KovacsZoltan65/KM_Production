...
const colors = ['#2563eb', '#059669', '#d97706', '#dc2626', '#7c3aed', '#0f766e', '#64748b'];
...
const segments = computed(() => {
    ...
    return props.rows.map((row, index) => {
        ...
        const segment = { ...row, color: colors[index % colors.length], dasharray: `${length} ${100 - length}`, dashoffset: -offset };
        offset += length;
...
<template>
    <div class="grid gap-4 sm:grid-cols-[9rem_1fr] sm:items-center">
        <div class="relative h-36 w-36">
            ...
            <div class="absolute inset-0 grid place-items-center text-xl font-semibold">{{ total }}</div>
        </div>
        <div class="space-y-2">
            ...
            <div v-if="segments.length === 0" class="text-sm text-slate-500">No data</div>
        </div>
    </div>
</template>
