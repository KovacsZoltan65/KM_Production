...
const readJson = (locale, filePath) => {
    try {
        ...
        console.error(
            `Invalid JSON in ${path.relative(root, filePath)} (${locale}): ${error.message}`,
        );
        process.exitCode = 1;
        return null;
    }
...
const translations = Object.fromEntries(
    Object.entries(files).map(([locale, filePath]) => [
        locale,
        readJson(locale, filePath),
    ]),
);
...
if (missingReferences.length) {
    console.error(
        `Missing static translation references (${missingReferences.length}):`,
    );
