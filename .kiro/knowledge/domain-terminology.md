...
| `Purchase Requisition Consolidation` | beszerzésiigény-konszolidáció       | Explicit execution use case, amely approved Purchase Supply Proposalokat dokumentált supplier- és időkulcs szerint Draft Purchase Requisition dokumentumokba csoportosít. Nem supplier selection, quantity policy vagy automatikus Proposal→PR átmenet. |
| `Procurement Source`                 | beszerzési forrás                   | Egy Item és Supplier közötti authoritative `ItemSupplier` kapcsolat, amely az alkalmazhatósági és beszerzési feltételeket tárolja. Nem supplier-választás, rendelés vagy supply promise.                                                                |
| `Supplier Candidate`                 | beszállítójelölt                    | Egy konkrét sourcing döntés minden érintett Itemjéhez az effective date-en eligible Procurement Source-okkal rendelkező Supplier. Nem automatikusan kiválasztott vagy „legjobb” Supplier.                                                               |
| `Supplier Selection`                 | beszállítóválasztás                 | Explicit, auditált döntés egy supplierless Draft Purchase Requisition minden tételéhez közös eligible Supplier candidate-ek közül. Nem ranking, quantity policy, PR approval vagy Purchase Order létrehozás.                                            |
| `Planned Quantity`                   | tervezett mennyiség                 | Az approved planning demand Purchase Requisitionbe vitt, változatlan base-unit pillanatképe. Konszolidált PR Itemnél a Proposal source quantityk exact összege; nem supplier constraint szerinti rendelési mennyiség.                                   |
| `Replenishment Quantity`             | utánpótlási / beszerzendő mennyiség | A kiválasztott ItemSupplier MOQ és order multiple feltételei után kapott requested base-unit mennyiség. Nem írja át a Material Requirement, netting, pegging vagy Supply Proposal quantityt.                                                            |
| `Replenishment Excess`               | utánpótlási többlet                 | `Replenishment Quantity - Planned Quantity`; supplier constraint miatt keletkező, mindig nem negatív többlet. Nem új Requirement vagy Proposal source quantity.                                                                                         |
...
| `Pegging`                            | szükséglet-visszakapcsolás          | Konkrét Supply és Requirement közötti mennyiségi planning trace. Megmagyarázza a netting coverage-et, de nem fizikai Reservation vagy Stock Movement.                                                                                                   |
...
| `Replenishment Strategy`             | utánpótlási stratégia               | A 0013 quantity-policy kontextusában a selected source exact/MOQ/order-multiple szabálya; tágabb stock-policy kontextusban később meghatározhatja a replenishment indító jelét és készletcélját. Nem Supply Strategy és V1-ben nem safety-stock engine. |
...
| `Preferred Supplier`                 | preferált beszállító                | Az Itemhez tartozó procurement source-ok közül üzletileg preferáltként jelölt supplier. Magyarázó signal, nem automatikus kiválasztás, kizárólagosság vagy approval.                                                                                    |
...
| `Execution Snapshot`                 | végrehajtási pillanatkép            | A PO létrehozásakor rögzített, történetileg stabil supplier-, item-, source-, unit-, conversion-, price- és policyadat. A későbbi master-data változás nem írhatja át.                                                                                  |
